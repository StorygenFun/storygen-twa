...
В ответе не должно быть ничего, кроме этого JSON.'`,
    ...
    sceneWrite: 'Максимально подробно напиши эпизод №{{num}} без указания номера этого эпизода',
    ...
    storySummaryGenerator: `У меня написана такая история: {{context}}
...
storyTitles - массив из 10 названий истории на русском языке (пример: ["name1", "name2", ... "name10"]).`,
    sceneVolume: 'Напиши текст объёмом {{from}}-{{to}} символов без указания номера этого эпизода.',
  },
  notFound: {
    stories: {
